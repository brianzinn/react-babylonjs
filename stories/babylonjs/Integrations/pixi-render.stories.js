...
const SceneBabylonJSSlider = (props) => <>
  <freeCamera name='camera1' position={new Vector3(0, 5, -10)} rotation={new Vector3(0, 0, 0)} />
  <hemisphericLight name='light1' intensity={0.7} direction={Vector3.Up()} diffuse={Color3.FromHexString("#ffffff")} groundColor={Color3.FromHexString("#7528FB")} />
  <Slider {...props} />
</>
...
export const PIXIStory = ({ top, mouse }) => {
  ...
  const onScroll = e => {
    return e.target && e.target.scrollTop ? state.top.current = e.target.scrollTop:false;
  }
  ...
  return (
    <div style={{ flex: 1, display: 'flex' }}> 
      ...
      <div className='slideContainer'>
        Slides: 
        <div className='btn' onClick={() => scrollToArea(0)}>1</div>
        <div className='btn' onClick={() => scrollToArea(1.5)}>2</div>
        <div className='btn' onClick={() => scrollToArea(2.2)}>3</div>
        <div className='btn' onClick={() => scrollToArea(3.2)}>4</div>
        <div className='btn' onClick={() => scrollToArea(4)}>5</div>
      </div>
