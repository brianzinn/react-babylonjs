  ...
  return (
    <>
      <div style={{ flex: 1, display: 'flex' }}>
        <button onClick={() => setLookAtPosition(new Vector3(0, lookAtPosition.y >= 2 ? -2 : lookAtPosition.y + 0.5, 0))}>Look Up, Down</button>
      </div>
      <div style={{ flex: 1, display: 'flex' }}>
        <Engine antialias adaptToDeviceRatio canvasId='babylonJS'>
          <Scene>
            ...
            <icoSphere name={'target'} radius={0.2} flat subdivisions={1} position={lookAtPosition}>
              <standardMaterial name={`target-mat`} diffuseColor={Color3.Red()} specularColor={Color3.Black()} />
            </icoSphere>
