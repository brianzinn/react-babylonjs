...
export default { title: "Non-Declarative" };
...
function onSceneMount(e) {
  ...
  var bgCamera = new ArcRotateCamera(
    "BGCamera",
    Math.PI / 2 + Math.PI / 7,
    Math.PI / 2,
    100,
  ...
  params.forEach(function (param) {
    ...
    header.text =
      param.name + ":" + pipeline.depthOfField[param.name].toFixed(2);
    header.height = "40px";
    header.color = "black";
  ...
  scene.onBeforeRenderObservable.add(function () {
    if (moveFocusDistance) {
      pipeline.depthOfField.focusDistance =
        600 + (4000 * (Math.sin(new Date().getTime() / 1000) + 1)) / 2;
    }
  });
...
export const NonDeclarativeStory = () => (
