...
export type CustomProps = {
  ...
  onControlAdded?: (instance: DecoratedInstance<any>) => void
  /**
...
export interface HostInstanceMetadata extends InstanceMetadataParameter {
  className: string
}
 ...
 */
export type ReactBabylonState<T> = {
  metadata: HostInstanceMetadata
  parent: DecoratedInstance<any> | null // Not the same as parent in BabylonJS, this is for internal reconciler structure. ie: graph walking
  children: DecoratedInstance<any>[]
  state?: any
...
export type DecoratedInstance<T> = T & {
  __rbs: ReactBabylonState<T>
};
