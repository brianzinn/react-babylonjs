...
export const useHover = (over?: HoverType, out?: HoverType): [MutableRefObject<Mesh | Control | null>, boolean] => {
  ...
  const ref = useRef<Mesh | Control | null>(null) as MutableRefObject<Mesh | Control | null>;
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      if (ref.current instanceof Mesh) { // TODO: instead check __rbs.metadata.isMesh === true
      ...
      } else if (ref.current instanceof Control) { // .metadata.isGUI2DControl === true
...
export function useClick(onClick: MeshEventType): [MutableRefObject<Mesh | null>] {
  ...
  useEffect(() => {
    if (ref.current) {
      if (ref.current instanceof Mesh) { // TODO __rbs.metadata.isMesh === true
        const mesh = ref.current as Mesh;
