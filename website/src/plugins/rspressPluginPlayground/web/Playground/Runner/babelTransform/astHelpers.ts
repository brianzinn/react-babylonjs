    ...
    declarations: [
      {
        ...
        id: typeof id === 'string' ? createIdentifier(id) : id,
        init,
      },
    ],
    kind: 'const',
...
export function createObjectPattern(names: (string | [string, string])[]): ObjectPattern {
  return {
    type: 'ObjectPattern',
...
export function createGetImport(name: string, getDefault?: boolean): CallExpression {
  return {
    type: 'CallExpression',
    callee: {
      type: 'Identifier',
