...
function CustomPlaneBlock(props) {
  ...
  useEffect(() => {
    const observable = scene.onBeforeRenderObservable.add((scene) => {
      if (scene !== null && planeRef !== null) {
        ...
        shMat.setFloat(
          "shift",
          lerp(shMat._floats.shift, (top.current - last) / 150, 0.1)
        );
        last = top.current;
      }
    });
  ...
  const onCustomProceduralTextureCreated = (e) => {
    ...
    const shMat = new ShaderMaterial(
      ...
      scene,
      {
        vertexSource: Effect.ShadersStore.shaderShiftRGBVertexShader,
        fragmentSource: Effect.ShadersStore.shaderShiftRGBPixelShader,
      },
      options
    );
  ...
  return (
    <plane
      ref={planeRef}
      name={`plane${keyText}-${index}`}
      scaling={scaling}
      onCreated={(e) => onCustomProceduralTextureCreated(e)}
    />
  );
}
  ...
  return (
    <Block factor={factor} offset={offset} name={`block${keyText}-${index}`}>
      <CustomPlaneBlock
        ...
        keyText={keyText}
        index={index}
        scaling={new Vector3(w * size, (w * size) / aspect, 1)}
      />
      <plane
        name={`dialog${keyText}-${index}`}
        scaling={new Vector3(w * size, (w * size) / aspect, 1)}
        size={w * 0.04}
        position={
          new Vector3(
      ...
      <plane
        name={`dialogText${keyText}-${index}`}
        scaling={new Vector3(w * size, (w * size) / aspect, 1)}
        size={w * 0.04}
        position={new Vector3(-w / 8, -w / 4 / aspect, -4)}
        sideOrientation={Mesh.BACKSIDE}
      >
        <advancedDynamicTexture
  ...
  return (
    <>
      ...
      {state.paragraphs.map((props, index) => {
        return <Paragraph key={index} index={index} {...props} />;
      })}
