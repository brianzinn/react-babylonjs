...
revolution of _yaw_ is _pi _ 2\*. Then, we'd need to know how many milliseconds
have passed between animation frames. Fortunately Babylon.js provides a
`getDeltaTime()` that tells us exactly that. Using all that information, we can
do this instead:
...
Now, in the interests of completeness, you might notice the hook is referring to
...
It's a little silly to require `rpm={5}` just to use the box. Let's make a
