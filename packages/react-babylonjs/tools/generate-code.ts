...
import {
  ...
  CreationType,
  GeneratedParameter,
} from "../src/codeGenerationDescriptors";
...
// issues #137, 148,
const fullySpecified = (moduleSpecifier: string): string =>
  `${moduleSpecifier}.js`;
 ...
 */
const LATE_BOUND_CONSTRUCTOR_PARAMETERS: Map<string, string[]> = new Map<
  string,
  string[]
>([
...
const CONFLICT_INTRINSIC_ELEMENTS = [
  "Button",
  "Ellipse",
  "Image",
  "Line",
...
const addHostElement = (
  className: string,
  babylonjsClassDeclaration: ClassDeclaration
): void => {
  if (!REACT_EXPORTS.has(className)) {
    ...
    const moduleDeclaration = getModuleDeclarationFromClassDeclaration(
      babylonjsClassDeclaration
    );
...
classesToGenerate.forEach((className) =>
  classesOfInterest.set(className, undefined)
);
...
const importDeclarationMap: Map<
  string,
  Map<string, FileModuleDeclaration[]>
> = new Map<string, Map<string, FileModuleDeclaration[]>>();
...
const getModuleDeclarationFromClassDeclaration = (
  ...
  const match = sourceFile
    .getFilePath()
    .match(`node_modules/(.*)${sourceFile.getExtension()}`);
  ...
  if (match === null || match.length < 2) {
    if (className !== "DynamicTerrain") {
      throw new Error(
        "Unknown (not from @babylonjs) class being generated:" + className
      );
    }
...
const addNamedImportToFile = (
  ...
  targets.forEach((file) => {
    ...
    const existingFileMap: Map<string, FileModuleDeclaration[]> =
      importDeclarationMap.get(fileKey)!;
...
const addSourceClass = (
  ...
  const findByKeyword = classesOfKeywordInterest.some((keyword) =>
    className.includes(keyword)
  );

  if (classesOfInterest.has(className) || findByKeyword) {
    const moduleDeclaration =
      getModuleDeclarationFromClassDeclaration(classDeclaration);
...
const addProject = (
  ...
  project.getSourceFiles().forEach((sourceFile: SourceFile) => {
      ...
      ns.getInterfaces().forEach(
        (interfaceDeclaration: InterfaceDeclaration) => {
          if (
            ...
            monkeyPatchInterfaces.set(interfaceDeclaration.getName(), [
              interfaceDeclaration,
            ]);
          }
          if (
            ...
            interfaceDeclaration
              .getProperties()
              .find((prop) => prop.getName() === "instancedBuffers")
          ) {
            monkeyPatchInterfaces.set(interfaceDeclaration.getName(), [
              interfaceDeclaration,
            ]);
    ...
    sourceFile.getEnums().forEach((enumDeclaration: EnumDeclaration) => {
      if (
        !enumDeclaration ||
        enumDeclaration.getType().getSymbol() === undefined
      ) {
        console.log("UNDEF enumdeclaration?");
      } else {
        const fqn: string = enumDeclaration
          .getType()
          .getSymbol()!
          .getFullyQualifiedName();
        const translatedType = createTypeFromText(
          fqn,
          sourceFiles,
...
const createFactoryClass = (
  ...
  let factoryBuilderTuple: ClassNameSpaceTuple =
    classesOfInterest.get(factoryClassName)!;

  let factoryMethods: MethodDeclaration[] =
    factoryBuilderTuple.classDeclaration.getStaticMethods();

  for (const method of factoryMethods) {
    const methodName: string = method.getName();
    if (
      (methodName && methodName.startsWith("Create")) ||
      methodName.startsWith("Extrude")
    ) {
      ...
      let newClassDeclaration: ClassDeclaration =
        addClassDeclarationFromFactoryMethod(
...
const addClassDeclarationFromFactoryMethod = (
  ...
  if (jsDocs.length > 0) {
    ...
    newClassDeclaration.addJsDoc(
      jsDocs[0].compilerNode.comment + "\n\n" + generatedComment
    );
  } else {
  ...
  const newConstructor: ConstructorDeclaration =
    newClassDeclaration.addConstructor();
  newConstructor.setBodyText((writer: CodeBlockWriter) => {
    ...
    let handlerClassDeclaration: ClassDeclaration | undefined =
      classDeclaration;
...
const includeAsConstructorParameter = (parameterType: string): boolean => {
  ...
  return !parameterType.includes(
    "BabylonjsCoreScene"
  ); /* provided by reconciler */
};
...
const getExpandedPropsFromParameter = (
  ...
  parameter
    ...
    .forEach((member) => {
      ...
      let internalType = member.getTypeAtLocation(
        member.getValueDeclaration()!
      );

      let propertyType = createTypeFromText(
        internalType.getText(),
        targetFiles
      );

      // let questionToken2 = (member.getValueDeclaration()!.compilerNode as any).questionToken;
      let questionToken = (member.compilerSymbol.valueDeclaration as any)
        .questionToken;
...
const parameterShouldBeExpanded = (
  parameter: ParameterDeclaration
): boolean => {
  const name = parameter.getName()!;
  const parameterType: string = createTypeFromText(
...
const addCreateInfoFromFactoryMethod = (
  ...
  method
    .getParameters()
    .forEach((createMethodParameter: ParameterDeclaration) => {
      ...
      const parameterType: string = createTypeFromText(
        createMethodParameter.getType().getText(),
      ...
      if (parameterShouldBeExpanded(createMethodParameter)) {
        let specialProperties: GeneratedParameter[] =
          getExpandedPropsFromParameter(createMethodParameter, [
            generatedCodeSourceFile,
            generatedPropsSourceFile,
          ]);
  ...
  if (property.getType().isClass() === true) {
    ...
    if (className !== undefined && className.endsWith("Configuration")) {
      const classNamespaceTuple: ClassNameSpaceTuple = {
        ...
        moduleDeclaration: getModuleDeclarationFromClassDeclaration(
          typeClassDeclarations[0]
        ),
      };
      console.log(
        ` >> adding ${classDeclaration.getName()!}.${property.getName()} (read-only) property type '${typeClassDeclarations[0].getName()}'`
      );
      readonlyPropertiesToGenerate.set(
        typeClassDeclarations[0].getName()!,
        classNamespaceTuple
      );
    }
...
const getMethodType = (
  ...
  params.forEach((param) => {
    let type: string = createTypeFromText(
  ...
  const returnType =
    methodDeclaration.getReturnType().getText() === "void"
      ? "void"
      : createTypeFromText(
          methodDeclaration.getReturnType().getText(),
          targetFiles
        );
...
const createClassDeclaration = (
  ...
  const baseClass: ClassDeclaration | undefined =
    classDeclaration.getBaseClass(); // no mix-ins in BabylonJS AFAIK, but would otherwise use baseTypes()
  const className: string = classDeclaration.getName()!;

  const importedClassDeclaration =
    getModuleDeclarationFromClassDeclaration(classDeclaration);
  ...
  newConstructor.setBodyText((writer: CodeBlockWriter) => {
    ...
    while (baseDeclaration !== undefined) {
      propsHandlers.push(
        `new ${CLASS_NAME_PREFIX}${baseDeclaration.getName()}PropsHandler()`
      );
    ...
    propertiesToAdd
      ...
      .forEach(
        (
          ...
          const paramDeclaration: ParameterDeclaration | undefined =
          ...
          if (paramDeclaration !== undefined) {
            const paramDescendants =
              paramDeclaration.forEachDescendantAsArray();
            ...
            if (
              propertyDescendantsParametersAndExpressions.every(
                (desc) =>
                  Node.isExpression(desc) ||
                  Node.isTypeReferenceNode(desc) /* ie: Nullable<> */
              )
            ) {
              allPrimitives =
                propertyDescendantsParametersAndExpressions.reduce<boolean>(
                  (result, expression) => result && isPrimitiveType(expression),
                  true
                );
            }
...
const writeTypeAlias = (
  ...
  const aliasType: WriterFunction =
    intersectsWithTypes.length > 0
      ? intersectionType(
          propertiesObject,
          intersectsWithTypes[0],
          ...intersectsWithTypes.slice(1)
        )
      : propertiesObject;
...
const addCreateInfoFromConstructor = (
  ...
  const forceOptionalParameters: string[] =
    LATE_BOUND_CONSTRUCTOR_PARAMETERS.has(sourceClass.getName()!)
  ...
  if (constructorDeclarations.length > 1) {
    console.error(
      "found multiple constructors (using first):",
      sourceClass.getName()
    );
  }
  ...
  if (constructorDeclarations.length > 0) {
    ...
    constructorDeclarations[0]
      ...
      .forEach((parameterDeclaration) => {
        const type: string = createTypeFromText(
          parameterDeclaration.getType().getText(),
          [generatedCodeSourceFile, generatedPropsSourceFile]
        );
        const name = parameterDeclaration.getName()!;
        const optional: boolean =
          parameterDeclaration.isOptional() ||
          forceOptionalParameters.indexOf(name) >= 0;
        ...
        if (includeAsConstructorParameter(type)) {
          if (parameterShouldBeExpanded(parameterDeclaration)) {
            let specialProperties: GeneratedParameter[] =
              getExpandedPropsFromParameter(parameterDeclaration, [
                generatedCodeSourceFile,
                generatedPropsSourceFile,
              ]);
            generatedParameter.type = specialProperties;
...
const createClassesDerivedFrom = (
  ...
  let classDeclaration: ClassDeclaration | undefined =
    classNamespaceTuple.classDeclaration;
  ...
  while (classDeclaration !== undefined) {
    ...
    inheritsFromNode =
      inheritsFromNode ||
      (classDeclaration !== undefined && classDeclaration.getName() === "Node");
  }
  ...
  for (let i = 0; i < classesToCreate.length; i++) {
    ...
    let baseClassDeclarationForCreate =
      baseClassDeclaration === undefined
        ? classDeclaration
...
const createClassesInheritedFrom = (
  ...
  const derivedClassesOrdered: Map<string, ClassDeclaration> = new Map<
    string,
    ClassDeclaration
  >();

  orderedListCreator.addDescendantsOrdered(
    [baseClassDeclaration],
    derivedClassesOrdered
  );
  ...
  derivedClassesOrdered.forEach((derivedClassDeclaration: ClassDeclaration) => {
    ...
    const metadata: InstanceMetadataParameter | undefined =
    ...
    addedClassDeclarationsMap.set(
      derivedClassDeclaration.getName()!,
      derivedClassDeclaration
    );
  });
...
const addReactExports = (generatedCodeSourceFile: SourceFile) => {
  ...
  // );
  const exportVars: VariableStatement =
...
const createSingleClass = (
  ...
  addHostElement(
    classToGenerate.classDeclaration.getName()!,
  ...
  console.log(
    ` > ${classToIntrinsic(classToGenerate.classDeclaration.getName()!)}`
  );
};
...
const generateCode = async () => {
  ...
  const extra = (
    ...
    let baseDeclaration: ClassDeclaration | undefined =
  ...
  if (classesOfInterest.get("Control")) {
    const instanceMetadaFromClassName = (
      className: string
  ...
  readonlyPropertiesToGenerate.forEach(
    (value: ClassNameSpaceTuple, className: string) => {
  ...
  Array.from(importDeclarationMap.values()).forEach((file) => {
    Array.from(file.values()).forEach(
      (fileModuleDeclarations: FileModuleDeclaration[]) => {
        const sourceFile: SourceFile = fileModuleDeclarations[0].sourceFile;
        const importDeclaration: ImportDeclaration =
          sourceFile.addImportDeclaration({
            moduleSpecifier:
              fileModuleDeclarations[0].moduleDeclaration.moduleSpecifier,
          } as ImportDeclarationStructure);

        fileModuleDeclarations.forEach(
          (fileModuleDeclaration: FileModuleDeclaration) => {
  ...
  const functionDeclaration: FunctionDeclaration =
    generatedCodeSourceFile.addFunction({
      isExported: true,
