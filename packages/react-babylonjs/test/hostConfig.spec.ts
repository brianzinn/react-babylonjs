...
describe(" > Reconciler tests", function testSuite() {
  ...
  it("Create box (Mesh) should be assigned AbstractMeshLifecycleListener", async function test() {
    ...
    const createdInstance: CreatedInstance<any> | undefined =
      renderer.createInstance(
        "box",
        {
          name: "box",
    ...
    assert.ok(
      createdInstance?.lifecycleListener instanceof
        AbstractMeshLifecycleListener,
  ...
  it("assignFrom should attach properly to parent property and assign deferred props (ie: without a prop update or re-render)", async function test() {
    ...
    const pbrClearCoatConfiguration:
      | CreatedInstance<PBRClearCoatConfiguration>
      | undefined = renderer.createInstance(
      "pbrClearCoatConfiguration",
      {
        assignFrom: "clearCoat",
        isEnabled: true,
    ...
    const pbrMaterial: CreatedInstance<PBRMaterial> | undefined =
      renderer.createInstance(
        "pbrMaterial",
        {
          name: "material",
  ...
  it("targetable Camera should be locked to target when specified.", async function test() {
    ...
    const freeCamera: CreatedInstance<FreeCamera> | undefined =
      renderer.createInstance(
        "freeCamera",
        {
          name: "freeCamera",
  ...
  it("removing from rootContainer should remove recursively child nodes.", async function test() {
    ...
    const standardMaterial: CreatedInstance<StandardMaterial> =
      renderer.createInstance(
        "standardMaterial",
        {
          name: "boxMaterial",
    ...
    assert.strictEqual(
      1,
      box.children.length,
      "box should have 1 child (material)"
    );
    ...
    assert.strictEqual(
      2,
      ground.children.length,
      "ground should have 2 children (box + sphere)"
    );
