...
> {
  onMount(instance: CreatedInstance<AdvancedDynamicTexture>): void {
    ...
    if ((instance.customProps as ADTCustomProps).createForParentMesh) {
      ...
      const supportPointerMove =
        (this.props as any).supportPointerMove !== false ? true : false;
  ...
  addControls(instance: CreatedInstance<any>) {
    ...
    instance.children.forEach((child) => {
      if (child.metadata.isGUI2DControl === true) {
        if (instance.metadata.isGUI2DGrid === true) {
          ...
          (instance.hostInstance as Grid).addControl(
            child.hostInstance,
            gridRow,
            gridColumn
          );
    ...
    if (
      (instance.customProps as VirtualKeyboardCustomProps)
        .connectControlNames !== undefined &&
      Array.isArray(
        (instance.customProps as VirtualKeyboardCustomProps).connectControlNames
      )
    ) {
  ...
  connect(
    ...
    searchInstance.children.forEach((child) =>
      this.connect(keyboard, child, controlNames)
    );
  }
}
