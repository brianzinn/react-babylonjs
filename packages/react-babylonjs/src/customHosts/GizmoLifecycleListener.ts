...
export default class GizmoLifecycleListener extends BaseLifecycleListener<
  Gizmo,
  GizmoCustomProps
> {
  onMount(instance: CreatedInstance<Gizmo>) {
    ...
    if (gizmoProps.skipUtilityLayerAttach !== true) {
      ...
      if (foundUtilityLayerRender !== true) {
        console.warn(
          "utility layer not found (if intentional use skipUtilityLayerAttach)"
        );
      }
    }
    ...
    if (
      ...
      (gizmoProps.attachGizmoToMesh !== false &&
        gizmoProps.attachGizmoToNode !== false)
    ) {
      const searchType =
        gizmoProps.attachGizmoToMesh === undefined &&
        gizmoProps.attachGizmoToNode === undefined
      ...
      while (tmp !== null) {
        ...
        if (
          (searchType === null || searchType === "mesh") &&
          tmp.metadata?.isMesh === true
        ) {
          gizmo.attachedMesh = tmp.hostInstance;
          break;
        }
        if (
          (searchType === null || searchType === "node") &&
          tmp.metadata?.isNode === true
        ) {
