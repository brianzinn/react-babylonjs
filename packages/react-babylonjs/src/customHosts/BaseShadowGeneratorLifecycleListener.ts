...
> extends DeferredCreationLifecycleListener<T, U> {
  ...
  createInstance = (
    ...
    if (
      ...
      this.onMeshAddedObservable = scene.onNewMeshAddedObservable.add(
        (mesh: AbstractMesh) => {
      ...
      this.onMeshRemovedObservable = scene.onMeshRemovedObservable.add(
        (mesh: AbstractMesh) => {
          if (shadowCasters.indexOf(mesh.name) >= 0) {
