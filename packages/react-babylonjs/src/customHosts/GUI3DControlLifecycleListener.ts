...
export default class GUI3DControlLifecycleListener extends BaseLifecycleListener<
  Control3D,
  any
> {
  ...
  addControls(
    instance: CreatedInstance<any>,
    last3DGuiControl: CreatedInstance<any>
  ) {
    ...
    instance.children.forEach((child) => {
      if (
        (last3d.customProps as Control3DCustomProps).childrenAsContent === true
      ) {
        ...
        if (
          ...
          const toLinkTo: Nullable<AbstractMesh> = this.scene!.getMeshByName(
            (child.customProps as Control3DCustomProps)
              .linkToTransformNodeByName!
          );
          if (toLinkTo !== null) {
            ...
            console.error(
              ...
              (instance.customProps as Control3DCustomProps)
                .linkToTransformNodeByName,
      ...
      instance.children.forEach((child) => {
        if (!child.state || child.state.content !== true) {
          if (
            child.state &&
            child.state.added === true &&
            typeof (child.customProps as Control3DCustomProps)
