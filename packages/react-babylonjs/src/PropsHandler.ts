...
export class CustomPropsHandler {
  private static _registeredPropsHandlers: Record<
    string,
  ...
  public static RegisterPropsHandler(
    ...
    const match = registeredHandlers.find(
      (h) => h === propsHandler || h.name === propsHandler.name
    );
    if (match !== undefined) {
  ...
  public static UnregisterPropsHandler(
    ...
    if (
      ...
      console.warn(
        `cannot find ${propsHandlerToUnregister.name} to unregister.`
      );
      return false;
    }
    ...
    CustomPropsHandler._registeredPropsHandlers[propsChangeType] =
      registeredHandlers.slice(index, 1);
    return true;
  }

  private static NOT_ACCEPTED: Readonly<HandlerUpdateProcessResult<any>> =
    Object.freeze({
      accepted: false,
      processed: false,
      value: null,
  ...
  private static ACCEPTED_NOT_PROCESSED: Readonly<
    HandlerUpdateProcessResult<any>
  > = Object.freeze({
    accepted: true,
    processed: false,
    value: null,
  ...
  public static HandlePropsChange(
    ...
    for (const handler of registeredHandlers) {
      if (handler.accept(newProp)) {
        accepted = true;
        const propertyUpdatedProcessResult: PropertyUpdateProcessResult<any> =
          handler.process(oldProp, newProp);
...
function propertyCheck<T>(
  ...
  try {
    const processedResult = CustomPropsHandler.HandlePropsChange(
      propChangeType,
      oldProp,
      newProp
    );
    ...
    console.error(
      `Unable to update '${propertyName}' with ${propChangeType}:`,
      newProp
    );
    console.error(e);
  }
