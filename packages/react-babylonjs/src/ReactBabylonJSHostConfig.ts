import {
  IInspectable,
  InspectableType,
} from "@babylonjs/core/Misc/iInspectable.js";
...
import {
  applyInitialPropsToCreatedInstance,
...
function removeChild(
  parentInstance: CreatedInstance<any>,
...
function addChild(
  ...
  if (
    ...
    parent.lifecycleListener.onChildAdded
  ) {
...
const getConstructorValue = (type: string, value: any): any => {
  switch (type) {
    ...
    case "BabylonjsCoreColor4": {
      const c4result = CustomPropsHandler.HandlePropsChange(
...
> & {
  ...
  prepareUpdate(
    ...
    if (
      !instance ||
      (instance.metadata && instance.metadata.customType === true)
    ) {
      return null;
    }
    ...
    instance
      .propsHandlers!.getPropsHandlers()
      .forEach((propHandler: PropsHandler<any>) => {
        const handlerUpdates: PropertyUpdate[] | null =
          propHandler.getPropertyUpdates(oldProps, newProps);
  ...
  insertBefore(
    ...
    if (parentInstance && beforeChild !== undefined) {
      index = parentInstance.children.indexOf(
        beforeChild as CreatedInstance<any>
      );
    }
  ...
  createInstance: (
    ...
    const underlyingClassName =
      (GENERATED.intrinsicClassMap as any)[type] || type;
    ...
    const customProps: AnyCustomProps = {
      ...
      onControlAdded:
        typeof props.onControlAdded === "function"
          ? props.onControlAdded
          : undefined,
    ...
    if (customProps.assignFrom !== undefined) {
      ...
      if (props.fromInstance !== undefined) {
        if (createInfoArgs.namespace.startsWith("@babylonjs/")) {
          ...
          if (props.fromInstance instanceof clazz) {
            ...
            customProps.disposeInstanceOnUnmount =
              props.disposeInstanceOnUnmount === true;
          } else {
          ...
          if (Array.isArray(generatedParameter.type)) {
            ...
            generatedParameter.type.forEach((subParameter) => {
              ...
              if (
                ...
                console.warn(
                  "Missing a required secondary property:",
                  subParameter.name
                );
              } else {
            ...
            let value = getConstructorValue(
              generatedParameter.type,
              props[generatedParameter.name]
            );
            if (value === undefined) {
              ...
              if (
                ...
                (generatedParameter.type === "any" &&
                  generatedParameter.name === "scene")
              ) {
        ...
        if (createInfoArgs.creationType === CreationType.FactoryMethod) {
          ...
          babylonObject = GENERATED.babylonClassFactory(
            createInfoArgs.libraryLocation
          )[createInfoArgs.factoryMethod!](...args);
        } else {
          if (metadata.delayCreation !== true) {
            if (createInfoArgs.namespace.startsWith("@babylonjs/")) {
              const clazz: any = GENERATED.babylonClassFactory(type);
    ...
    if (
      (CUSTOM_HOSTS as any)[underlyingClassName + "LifecycleListener"] !==
      undefined
    ) {
      lifecycleListener = new (CUSTOM_HOSTS as any)[
        underlyingClassName + "LifecycleListener"
      ](scene, props);
      ...
      lifecycleListener = dynamicRegisteredHost.lifecycleListenerFactory(
        scene!,
        props
      );
    } else {
      lifecycleListener = new CUSTOM_HOSTS.FallbackLifecycleListener(
        scene!,
        props
      );
    }
    ...
    if (
      createdReference.metadata &&
      createdReference.metadata.isTargetable === true
    ) {
      fiberObject.addPropsHandler(new CUSTOM_HOSTS.TargetPropsHandler(scene!));
    }

    if (
      metadata.delayCreation !== true &&
      customProps.assignFrom === undefined
    ) {
    ...
    if (
      ...
      !("metadata-className" in createdReference.hostInstance)
    ) {
  ...
  appendInitialChild: (
    parent: HostCreatedInstance<any>,
    child: CreatedInstance<any>
  ) => {
  ...
  commitMount: (
    ...
    if (
      ...
      instance.lifecycleListener.onMount
    ) {
