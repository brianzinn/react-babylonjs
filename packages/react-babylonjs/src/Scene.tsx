...
const updateScene = (
  ...
  const updates: UpdatePayload = propsHandler.getPropertyUpdates(
    prevProps,
    props
  );
...
const Scene: React.FC<SceneProps> = (props: SceneProps, context?: any) => {
  ...
  const prevPropsRef: MutableRefObject<Partial<SceneProps>> = useRef<
    Partial<SceneProps>
  >({});
  ...
  useEffect(
    () => {
      ...
      if (sceneIsReady) {
        ...
        console.error(
          "Scene is not ready. Report issue in react-babylonjs repo"
        );
      }
  ...
  useEffect(() => {
    ...
    updateScene(
      props,
      prevPropsRef,
      containerRef.current!.rootInstance,
      propsHandler
    );
