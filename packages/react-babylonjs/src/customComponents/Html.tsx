...
function isObjectBehindCamera(el: AbstractMesh, camera: Camera) {
  ...
  const outsideFrustum =
    planes.findIndex((x) => x.dotCoordinate(center) < 0) === -1 ? false : true; // taken from : https://forum.babylonjs.com/t/check-if-vector3-is-in-frustum/8652/3
...
function isObjectVisible(
  ...
  let hit = camera
    ...
    .pickWithRay(
      ray,
      (m) =>
        (occlude.length > 0 ? occlude.indexOf(m) !== -1 : m !== el.parent) &&
        m.name !== "skybox"
    );
  ...
  return hit
    ? hit.pickedMesh?.name === el.parent?.name ||
        hit.distance * hit.distance >=
          Vector3.DistanceSquared(objectPos, cameraPos)
    : false;
}
  ...
  (
    ...
    useBeforeRender(() => {
      ...
      if (camera && group.current) {
        ...
        const vec = transform
          ? oldPosition.current
          : calculatePosition(node, camera);

        el.style.display =
          visible.current && node.isEnabled(true) ? "block" : "none";
        ...
        if (
          ...
          let raytraceTarget: null | undefined | boolean | AbstractMesh[] =
            false;
          ...
          if (transform) {
            ...
            const { mode, orthoTop, orthoLeft, orthoBottom, orthoRight } =
              camera;
            ...
            const cameraTransform =
              mode === Camera.ORTHOGRAPHIC_CAMERA
                ? `scale(${fov})translate(${epsilon(
                    -(orthoRight! + orthoLeft!) / 2
                  )}px,${epsilon((orthoTop! + orthoBottom!) / 2)}px)`
                : `translateZ(${fov}px)`;
            ...
            el.style.perspective =
              mode === Camera.ORTHOGRAPHIC_CAMERA ? "" : `${fov}px`;
            ...
            const scale =
              distanceFactor === undefined
                ? 1
                : objectScale(node, camera) * distanceFactor;
