...
export const AssetManagerContext =
  React.createContext<AssetManagerContextType>(undefined);
...
export const AssetManagerContextProvider: React.FC<
...
const useAssetManagerWithCache = (): ((
  ...
  return (tasks: Task[], options?: AssetManagerOptions): AssetManagerResult => {
    ...
    const assetManagerContext =
      useContext<AssetManagerContextType>(AssetManagerContext);
    ...
    const createGetAssets = (tasks: Task[]): (() => AssetManagerResult) => {
      ...
      const newRequests: Map<AbstractAssetTask, Task> = new Map<
        AbstractAssetTask,
        Task
      >();
      ...
      tasks.forEach((task) => {
        ...
        if (key !== undefined && suspenseCache[key]) {
          ...
          switch (task.taskType) {
            case TaskType.Binary:
              const binaryTask = assetManager.addBinaryFileTask(
                task.name,
                task.url
              );
            ...
            default:
              throw new Error(
                `Only binary/mesh supported currently.  'taskType' found on ${JSON.stringify(
                  task
                )}`
              );
          }
      ...
      const taskPromise =
        tasks.length === cachedTasks.length
          ...
          : new Promise<AssetManagerResult>((resolve, reject) => {
              let failed = false;
              assetManager.useDefaultLoadingScreen =
                opts.useDefaultLoadingScreen ?? false;
              assetManager.onFinish = (tasks: AbstractAssetTask[]) => {
                ...
                if (failed === false) {
                  ...
                  const result = createResultFromTasks(
                    tasks.concat(cachedTasks)
                  );
                  resolve(result);
                }
              };
