...
export const useCustomPropsHandler = (
  propsHandler: ICustomPropsHandler<
    any,
    any
...
export const useHover = (
  ...
  const createdRef = useRef<Mesh>(null) as MutableRefObject<
    Mesh | Control | null
  >;
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      if (ref.current instanceof AbstractMesh) {
        ...
        const onPointerOverAction: Nullable<IAction> =
        ...
        const onPointerOutAction: Nullable<IAction> =
          mesh.actionManager.registerAction(
            new ExecuteCodeAction(
        ...
        observer2dGuiEnter = control.onPointerEnterObservable.add(
          over as Gui2dEventType
        );
        observer2dGuiOut = control.onPointerOutObservable.add(
          out as Gui2dEventType
        );
      } else {
        console.warn(
          "Can only apply useHover to non-mesh/2D control currently.",
          ref.current
        );
      }
